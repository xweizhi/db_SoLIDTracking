...
int main( int argc, const char** argv )
{
  ...
  const int    nsystem   = 1; //PVDIS has 30 independent tracker system
  const int    nplanes   = 6;  //each system has 5 GEM plane
  const int    nchamber  = 30;  //each plane has 1 GEM chamber
  ...
  double phi_offset[nplanes] = {0.};
  
  //nominal z position of each GEM plane
  double plane_z[nplanes]    = {-1.75, -1.50, -1.19, -0.68, 0.05, 0.92};
